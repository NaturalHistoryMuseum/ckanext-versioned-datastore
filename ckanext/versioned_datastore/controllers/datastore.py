...
class ResourceDataController(toolkit.BaseController):
    ...
    def resource_data(self, package_name, resource_id):
        ...
        if toolkit.request.method == u'POST':
            toolkit.get_action(u'datastore_reindex')({}, {u'resource_id': resource_id})
            toolkit.h.flash_success(toolkit._(u'Reindexing submitted, this may take a few minutes. You can monitor '
                            u'progress below'))
            ...
            toolkit.redirect_to(u'resource_data', package_name=package_name, resource_id=resource_id,
                        _code=303)
        else:
            extra_vars = {
