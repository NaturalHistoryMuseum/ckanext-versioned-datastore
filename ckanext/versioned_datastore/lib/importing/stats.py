...
def update_stats(stats_id, update):
    '''
    Update the ImportStats object with the given database id with the given update dict. The update
    dict will be passed directly to SQLAlchemy.
...
def monitor_ingestion(stats_id, ingester):
    '''
    Adds monitoring functions to the ingester and updates the ImportStats (with the given database
    id) when updates come through.
...
def monitor_indexing(stats_id, indexer, update_frequency=1000):
    '''
    Adds monitoring functions to the indexer and updates the ImportStats (with the given database
    id) when updates come through.
...
def mark_error(stats_id, error):
    '''
    Marks the ImportStats object with the given database id as having finished with an error. Just
    the error message is stored against the ImportStats object. "in_progress", "duration" and "end"
    are also updated.
...
def get_all_stats(resource_id):
    '''
    Retrieves and returns all the ImportStats from the database associated with the given resource.
    They are ordered by ID descending which will result in the newest results coming back first.
