...
def get_latest_version(resource_id):
    ...
    return common.SEARCH_HELPER.get_latest_index_versions([index_name]).get(index_name, None)
...
def is_datastore_resource(resource_id):
    ...
    # index
    return (common.ES_CLIENT.indices.exists(index_name) and
            index_name in common.SEARCH_HELPER.get_latest_index_versions([index_name]))
...
def is_ingestible(resource):
    ...
    resource is used for this, not the URL). If the url is None, False is returned. This is
    technically not possible due to a Resource model constraint but it's worth covering off anyway.
