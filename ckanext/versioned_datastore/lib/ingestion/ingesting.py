...
def ingest_resource(version, config, resource, data, replace, api_key):
    ...
    try:
        ...
        with InclusionTracker(ingester) as tracker:
            ...
            if replace:
                replace_feeder = ReplaceDeletionFeeder(version, resource_id, tracker, feeder.source)
...
def prepare_resource(resource, version, data=None, api_key=None):
    ...
    if not then the resource url is used and the data is extracted from the resource url.
...
def get_fp_and_reader_for_resource_data(resource, data=None, api_key=None):
    '''
    Context manager which given a resource, yields a file pointer to the downloaded resource source
    and a ResourceReader instance for reading the data from the file pointer.
    ...
    if data is None:
        ...
        if resource_format != u'zip':
            ...
            if reader is not None:
                ...
                with download_to_temp_file(url, compress=reader.compressible,
                                           headers=headers) as temp:
                    yield temp, reader
                    handled = True
        else:
            ...
            with download_to_temp_file(url, compress=False, headers=headers) as temp_download:
...
class DatastoreFeeder(IngestionFeeder):
    '''
    Ingestion feeder class for the versioned datastore. This feeder handles reading rows from the
    intermediate data file format produced by the prepare_resource function above.
    '''
    ...
    def iter_rows(self, skip_header_row):
        '''
        Returns a generator which can be used to iterate over the rows in the data file. This
        generator yields dicts and therefore handles reading and deserialising the rows so that you
        don't have to.
    ...
    def header(self):
        '''
        Property providing access to the header in the data file. This function caches the header
        dict in self._header for repeated use and thus avoids reading the file every time.
