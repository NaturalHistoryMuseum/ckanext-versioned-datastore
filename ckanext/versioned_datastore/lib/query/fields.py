...
def select_fields(all_fields, search, number_of_groups):
    ...
    for chunk in chunk_iterator(all_fields.get_searches(search), chunk_size=number_of_groups):
        ...
        for (group, count, fields), response in zip(groups, multisearch.execute()):
            if all_fields.is_forced(group) or response.hits.total > 0:
                ...
                selected_fields.append(dict(group=group, count=count, records=response.hits.total,
                                            fields=fields, forced=all_fields.is_forced(group)))
...
def get_single_resource_fields(all_fields, resource_id, version, search, number_of_groups):
    '''
