...
def create_sv_writer(field_names, dialect, target_dir, filename):
    '''
    Creates a file object and a reader that can be used to write data into the given file name and
    then returns both.
...
def sv_writer(request, target_dir, field_counts, dialect, extension):
    '''
    Provides the functionality to write data to *sv files (for example: csv and tsv). This function
    handles opening and closing the necessary files to write data to either one file or a file
    per resource.
