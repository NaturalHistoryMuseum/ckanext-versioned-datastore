...
class DwcDerivativeGenerator(BaseDerivativeGenerator):
    ...
    def __init__(self, output_dir, fields, query, resource_id=None, core_extension_name=None,
                 extension_names=None, extension_map=None, id_field='_id', **format_args):
        super(DwcDerivativeGenerator, self).__init__(output_dir, fields, query, resource_id, **format_args)
        ...
        if core_extension_name is not None and core_extension_name.lower() in urls.core_extensions:
        ...
        if extension_names is None:
            extension_names = [e.strip().lower() for e in
                               toolkit.config.get('ckanext.versioned_datastore.dwc_extension_names',
                                                  '').split(',')]
        ...
        self.file_paths = {
            'core': os.path.join(self._build_dir, f'{self.schema.row_type_name.lower()}.csv')}
        ...
        self.fields = {'core': ['_id'] + sorted(list(set(utils.standard_fields + dataset_fields)))}

        for e in self.schema.extensions:
            self.file_paths[e.name] = os.path.join(self._build_dir, f'{e.name.lower()}.csv')
            subfields = [f.split('.')[-1] for f in self.all_fields if
    ...
    def initialise(self):
        ...
        self.writers = {k: csv.DictWriter(self.files[k], self.fields[k], dialect='unix') for k in
                        self.fields}
    ...
    def validate(self, record):
        if 'type' in record and record['type'] not in utils.valid_types:
            self.writers['core'].fieldnames = [f for f in self.fields['core'] if f != 'type']
    ...
    def make_meta(self):
        ...
        :return: xml string
        '''
        nsmap = utils.NSMap(xsi=urls.XMLUrls.xsi,
                            xs=urls.XMLUrls.xs)
        root = etree.Element('archive', nsmap=nsmap,
                             attrib={'xmlns': urls.TDWGUrls.xmlns, 'metadata': 'eml.xml',
    ...
    def make_eml(self):
        ...
        resources = [resource_show({}, {'id': r}) for r in self._query.resource_ids_and_versions]
        packages = [package_show({}, {'id': r['package_id']}) for r in resources]

        # define some variables
        org = {'organizationName': utils.get_setting('ckanext.versioned_datastore.dwc_org_name',
        ...
        query_license = licenses[0] if len(licenses) == 1 else utils.get_setting(
        ...
        if self.resource_id:
            ...
            if plugin_loaded('query_dois'):
                ...
                query_doi = find_existing_doi(self._query.resource_ids_and_versions,
                                              self._query.hash, self._query.query_version)
                if query_doi:
                    dataset_metadata['alternateIdentifier'] += ['doi:' + query_doi.doi]
                    dataset_metadata['distribution'][0]['online']['url'] = (
                        site_url + toolkit.url_for('query_doi.landing_page',
                                                   data_centre=utils.get_setting(
        ...
        if self.schema.core_extension and self.schema.core_extension.location.base == urls.GBIFUrls.base_url:
        ...
        nsmap = utils.NSMap(eml=urls.XMLUrls.eml,
                            dc=urls.XMLUrls.dc,
